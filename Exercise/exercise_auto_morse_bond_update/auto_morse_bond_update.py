...
###########################################################################################################
topofile = 'SWCNT_dia_2nm_IFF.data'
...
###########################################################################################################
morsefile = 'Morse_parameters.txt'
...
###########################################################################################################
# Python dictionary to set element symbol from the mass of the atom if reading in a LAMMPS .data file.    #
# Each key in the dictionary signifies an element and must have a value of a list with at least one mass  #
# in the list. No examples are given since this is self-explanatory, however if a read file does cannot   #
# find the information needed from this dictionary and error will be issued and the code will exit.       #
#                                                                                                         #
#        element: [list of masses to identify element type from datafile                                  #
#        symbol :  minimally each list needs at least 1 mass in them]                                     #
###########################################################################################################
mass_map = {'C':   [12.0000, 12.01115, 12.01100, 12.0112, 10.01115, 10.0112],
            'H':   [1.0080, 1.00797, 1.00782, 1.0000],
            'O':   [15.9990, 15.99940, 15.99491, 15.994],
            'N':   [14.0000, 14.00670, 14.007],
            'S':   [32.06400],
            'F':   [18.9984],
            'Si':  [28.086, 28.06],
             }  
